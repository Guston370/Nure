...
public class ProductDetailsActivitySimple extends BaseActivity {
    ...
    private void populateBasicInfo() {
        ...
        if (productBrand != null)
            productBrand.setText(currentProduct.brand);
        if (productBarcode != null)
            productBarcode.setText("Barcode: " + currentProduct.barcode);
        if (servingSizeText != null)
            servingSizeText.setText("Per " + currentProduct.servingSize);
        if (ingredientsText != null)
            ingredientsText.setText(currentProduct.ingredients);
        if (allergensText != null)
            allergensText.setText(currentProduct.allergens);
    }
    ...
    private void calculateHealthScore() {
        ...
        categoryScores.add(
                new CategoryScoresAdapter.CategoryScore("Calorie Density", calorieScore, 20, getCalorieAnalysis()));

        // 2. Sugar Content Score (20 points)
        double sugarScore = calculateSugarScore(currentProduct.sugar);
        categoryScores
                .add(new CategoryScoresAdapter.CategoryScore("Sugar Content", sugarScore, 20, getSugarAnalysis()));
        ...
        categoryScores.add(
                new CategoryScoresAdapter.CategoryScore("Protein Content", proteinScore, 15, getProteinAnalysis()));

        // 5. Fiber Content Score (10 points)
        double fiberScore = calculateFiberScore(currentProduct.fiber);
        categoryScores
                .add(new CategoryScoresAdapter.CategoryScore("Fiber Content", fiberScore, 10, getFiberAnalysis()));

        // 6. Sodium Content Score (10 points)
        double sodiumScore = calculateSodiumScore(currentProduct.sodium);
        categoryScores
                .add(new CategoryScoresAdapter.CategoryScore("Sodium Content", sodiumScore, 10, getSodiumAnalysis()));

        // 7. Processing Level Score (10 points)
        double processingScore = calculateProcessingScore(currentProduct.ingredients);
        categoryScores.add(new CategoryScoresAdapter.CategoryScore("Processing Level", processingScore, 10,
                getProcessingAnalysis()));

        // Calculate total score
        double totalScore = calorieScore + sugarScore + fatScore + proteinScore + fiberScore + sodiumScore
                + processingScore;
    ...
    private double calculateProcessingScore(String ingredients) {
        ...
        String[] artificialTerms = { "artificial", "preservative", "color", "flavor enhancer", "stabilizer",
                "emulsifier" };
        ...
        if (artificialCount == 0)
            return 10;
        else if (artificialCount <= 2)
            return 7;
        else if (artificialCount <= 4)
            return 4;
        else
            return 0;
    }
    ...
    private String getHealthScoreDescription(double score) {
        if (score >= 85)
            ...
            return "Good - Healthy Option";
        else if (score >= 55)
            return "Fair - Moderate Choice";
        else if (score >= 40)
    ...
    private void populateNutritionalInfo() {
        ...
        if (caloriesValue != null)
            caloriesValue.setText(String.format("%.0f kcal", currentProduct.calories));
        if (proteinValue != null)
            proteinValue.setText(String.format("%.1fg", currentProduct.protein));
        if (sugarValue != null)
            sugarValue.setText(String.format("%.1fg", currentProduct.sugar));
        if (fatValue != null)
            fatValue.setText(String.format("%.1fg", currentProduct.fat));
        if (carbsValue != null)
            carbsValue.setText(String.format("%.1fg", currentProduct.carbs));
        if (sodiumValue != null)
            sodiumValue.setText(String.format("%.0fmg", currentProduct.sodium));
    }

    // Analysis methods for each category
    private String getCalorieAnalysis() {
        if (currentProduct.calories <= 100)
            return "Low calorie density - Great for weight management";
        else if (currentProduct.calories <= 300)
            return "Moderate calorie density - Reasonable portion control needed";
        else
            return "High calorie density - Consider smaller portions";
    }

    private String getSugarAnalysis() {
        if (currentProduct.sugar <= 5)
            return "Low sugar content - Excellent choice";
        else if (currentProduct.sugar <= 15)
            return "Moderate sugar content - Consume in moderation";
        else
            return "High sugar content - Limit consumption";
    }

    private String getFatAnalysis() {
        if (currentProduct.fat <= 3)
            return "Low fat content - Heart healthy";
        else if (currentProduct.fat <= 15)
            return "Moderate fat content - Watch portions";
        else
            return "High fat content - Consider alternatives";
    }

    private String getProteinAnalysis() {
        if (currentProduct.protein >= 15)
            return "High protein content - Great for muscle health";
        else if (currentProduct.protein >= 5)
            return "Moderate protein content - Good source";
        else
            return "Low protein content - Consider protein-rich alternatives";
    }

    private String getFiberAnalysis() {
        if (currentProduct.fiber >= 6)
            return "High fiber content - Excellent for digestion";
        else if (currentProduct.fiber >= 3)
            return "Good fiber content - Supports digestive health";
        else
            return "Low fiber content - Add more fiber-rich foods";
    }

    private String getSodiumAnalysis() {
        if (currentProduct.sodium <= 300)
            return "Low sodium content - Heart healthy choice";
        else if (currentProduct.sodium <= 1000)
            return "Moderate sodium content - Monitor daily intake";
        else
            return "High sodium content - Limit consumption";
    }
